...
export async function renderInvoicePdf(data: InvoiceData): Promise<Buffer> {
  ...
  doc
    ...
    .fillColor('#111')
    .font('Helvetica-Bold')
    .text(data.companyLegalName.toUpperCase(), MARGIN, headerTop, {
      width: CONTENT_W,
      align: 'right',
    });
  doc
    .font('Helvetica')
    .fontSize(9)
    .text(data.companyAddress, MARGIN, headerTop + 14, {
      width: CONTENT_W,
      align: 'right',
    });
  if (data.companyTpin) {
    doc
      .fillColor('#2563eb')
      .font('Helvetica-Bold')
      .text(`TPIN: ${data.companyTpin}`, MARGIN, headerTop + 28, {
        width: CONTENT_W,
        align: 'right',
      });
  }
  ...
  if (data.customerEmail) {
    doc
      .font('Helvetica')
      .text(data.customerEmail, MARGIN + 22, y, { width: 280 });
    y += 12;
  }
  ...
  doc
    .rect(tableX + colQty, y, colDesc, headerH)
    .fillAndStroke('#f8fafc', '#111');
  doc
    .rect(tableX + colQty + colDesc, y, colUnit, headerH)
    .fillAndStroke('#f8fafc', '#111');
  doc
    .rect(tableX + colQty + colDesc + colUnit, y, colAmt, headerH)
    .fillAndStroke('#f8fafc', '#111');
  ...
  doc.text('Unit Price', tableX + colQty + colDesc, y + 7, {
    width: colUnit - 4,
    align: 'right',
  });
  doc.text('Amount', tableX + colQty + colDesc + colUnit, y + 7, {
    width: colAmt - 4,
    align: 'right',
  });
  ...
  doc
    .moveTo(tableX, itemY)
    .lineTo(tableX + CONTENT_W, itemY)
    .strokeColor('#111')
    .stroke();

  doc.font('Helvetica').fontSize(9);
  doc.text('1', tableX, itemY + 6, { width: colQty, align: 'center' });
  doc.text(description, tableX + colQty + 6, itemY + 4, {
    width: colDesc - 10,
  });
  doc.text(paymentNote, tableX + colQty + 6, itemY + 16, {
    width: colDesc - 10,
    lineBreak: false,
  });
  doc
    .fontSize(7)
    .fillColor('#555')
    .text(`Ref: ${data.depositId}`, tableX + colQty + 6, itemY + 26, {
      width: colDesc - 10,
    });
  doc.fontSize(9).fillColor('#111');
  doc.text(data.grandTotal, tableX + colQty + colDesc, itemY + 8, {
    width: colUnit - 4,
    align: 'right',
  });
  doc.text(data.grandTotal, tableX + colQty + colDesc + colUnit, itemY + 8, {
    width: colAmt - 4,
    align: 'right',
  });
  ...
  doc.font('Helvetica').text(data.subTotal, totalsX + 74, y + 4, {
    width: boxW - 8,
    align: 'right',
  });
  ...
  drawBox(doc, totalsX + 70, y - 2, boxW, boxH);
  doc.font('Helvetica').text(data.vatAmount, totalsX + 74, y + 4, {
    width: boxW - 8,
    align: 'right',
  });
  ...
  drawBox(doc, totalsX + 70, y - 2, boxW, boxH);
  doc.font('Helvetica-Bold').text(data.grandTotal, totalsX + 74, y + 4, {
    width: boxW - 8,
    align: 'right',
  });
  ...
  doc
    ...
    .font('Helvetica-Bold')
    .text(data.status.toUpperCase(), MARGIN, y + 12, {
      width: 56,
      align: 'center',
    });
  ...
  if (data.companyPhones)
    doc.text(`Cell: ${data.companyPhones}`, MARGIN, y + 12);
  ...
  if (data.bankName || data.bankAccountNumber) {
    ...
    if (data.bankName)
      doc.text(`Bank Name: ${data.bankName}`, bankX, y, { width: 240 });
    if (data.bankBranch)
      doc.text(`Branch: ${data.bankBranch}`, bankX, y + 12, { width: 240 });
    doc.text(`Account Name: ${data.bankAccountName}`, bankX, y + 24, {
      width: 240,
    });
    if (data.bankAccountNumber)
      doc.text(`Account Number: ${data.bankAccountNumber}`, bankX, y + 36, {
        width: 240,
      });
    doc
      .moveTo(bankX, y + 52)
      .lineTo(PAGE_W - MARGIN, y + 52)
      .strokeColor('#2563eb')
      .lineWidth(2)
      .stroke();
  }
